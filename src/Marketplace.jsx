...
import { createMarketplaceListing, loadMarketplaceListings, markMarketplaceListingSold, uploadMarketplaceImages } from "./lib/marketplaceListings";
import { validateMarketplacePhotoSelection } from "./lib/marketplaceImages";
import { startMarketplaceConversation } from "./lib/communityMessaging";
 ...
 function startPosting(){if(session){setPosting(true);return}sessionStorage.setItem("airportAuthReturnTo","/marketplace");sessionStorage.setItem("airportPendingAction","post-marketplace");window.location.assign("/register")}
  ...
  async function submitListing(e){
    ...
    try {
      ...
      const images=await uploadMarketplaceImages(item.id, photos);
      const complete={...item,images};
      setMine(current=>[complete,...current]);
      setPhotos([]);
      setPosting(false);
  ...
  async function messageSeller(item){
    if(!session){
      sessionStorage.setItem("airportAuthReturnTo","/marketplace");
      window.location.assign("/register");
      return;
    }
    try{
      const conversationId=await startMarketplaceConversation(item.id);
      navigate(`/messages/${conversationId}`);
    }catch(err){alert(err.message||"Unable to start this conversation.")}
  }
